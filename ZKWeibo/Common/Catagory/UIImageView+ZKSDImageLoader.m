...
#import "NSString+ZKUtilities.h"
@implementation UIImageView (ZKSDImageLoader)

- (void)zk_sd_setImageWithURL:(NSString *)url placeholderImageName:(NSString *)placeholderImageName {
    [self zk_sd_setImageWithURL:url placeholderImageName:placeholderImageName cachePlachoderImage:YES];
}
...
- (void)zk_sd_setImageWithURL:(NSString *)url placeholderImageName:(NSString *)placeholderImageName cachePlachoderImage:(BOOL)cachePlachoderImage {
    if ([[SDImageCache sharedImageCache] diskImageExistsWithKey:url]) {
        ...
        [self sd_setImageWithURL:[url zk_encodedURL] placeholderImage:placeholderImage completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
